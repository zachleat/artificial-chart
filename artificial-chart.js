class ArtificialChart {
  ...
  static wrapText(text, width) {
    text.each(function() {
      ...
      if(wrapCount) {
        text.attr("dy", 0).attr("class", "artfc-label-wrapped");
  ...
  generateLegend(labels = []) {
    ...
    container.classList.add("artfc-legend");
    ...
    for(let j = 0; j < labels.length; j++) {
      ...
      entries.push({
        label: labels[j],
        html: `<${tag}${attrs} class="artfc-legend-entry artfc-legend-${j + this.options.colorMod}">${labels[j] || ""}</${tag}>`
      });
    }
  ...
  highlightElements(target, method) {
    ...
    if(target.classList.contains("artfc-legend-entry")) {
      ...
      circle.closest("svg").classList[method]("artfc-bubble-active");
    }
  }
  ...
  renderLegend(data) {
    ...
    let legendPlaceholderClass = "artfc-legend-placeholder";
    let childSelector = `:scope .${legendPlaceholderClass}`;

    let previousEl = this.target.previousElementSibling;
    let legendAnchorBefore;
    if(previousEl && previousEl.classList.contains(legendPlaceholderClass)) {
      legendAnchorBefore = previousEl;
    } else {
      legendAnchorBefore = previousEl ? previousEl.querySelector(childSelector) : null;
    }

    let nextEl = this.target.nextElementSibling;
    let legendAnchorAfter;
    if(nextEl && nextEl.classList.contains(legendPlaceholderClass)) {
      legendAnchorAfter = nextEl;
    } else {
      legendAnchorAfter = nextEl ? nextEl.querySelector(childSelector) : null;
    }
  ...
  render(chart, data) {
    ...
    svg.append("g")
      ...
      .join("rect")
        ...
        .attr("class", (d, j) => `artfc-color-${j + options.colorMod}`);
    ...
    if(options.showInlineBarValues) {
      svg.append("g")
        ...
        .join("text")
          ...
          .attr("class", "artfc-inlinebarvalue")
    ...
    if(options.showAxisLabels) {
      svg.append("text")
        ...
        .attr("y", height - 6)
        .attr("class", "artfc-axislabel artfc-axislabel-center")
...
class HorizontalBar extends ArtificialChart {
  ...
  render(chart, data) {
    ...
    if(options.showInlineBarValues) {
      svg.append("g")
        ...
        .join("text")
          ...
          .attr("class", d => "artfc-inlinebarvalue-h" + (options.showInlineBarValues.length ? ` ${options.showInlineBarValues}` : ""))
          .attr("fill", d => options.showInlineBarValues === "inside" ? labelColors(d.key) : "currentColor")
          .text(d => this.roundValue(d.value, options.valueType[0]) + (options.valueType[0] === "percentage" ? "%" : ""));
    }
    ...
    if(options.wrapAxisLabel && options.wrapAxisLabel.left) {
      ArtificialChart.wrapText(svg.selectAll(".artfc-yaxis .tick text"), margin.left - 6);
    }
  }
}
  ...
  render(chart, data) {
    ...
    circles
      .enter()
        ...
        .attr("class", (d, j) => `artfc-bubblecircle artfc-color-${j + options.colorMod}`);
    ...
    circles
      .enter()
        ...
        .attr("y", d => yScale(d.y))
        .attr("class", d => {
          return "artfc-bubblelabel" + (isOffsetLabel(d) ? " offset-l" : "");
        })
